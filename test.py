...
class MemoryManager:
    ...
    def next_fit(self, process_name, size):
        ...
        for i in range(self.total_memory):
            pos = (start + i) % self.total_memory
            if self.check_free_space(pos, size):
                self.allocate_memory(pos, process_name, size)
                self.last_allocation = (pos + size) % self.total_memory
                self.history.append(f"Asignado {process_name} (tamaño {size}) en posición {pos} - Siguiente Ajuste")
                return pos
...
class MemorySimulatorApp:
    ...
    def setup_ui(self):
        ...
        self.algorithm_var = tk.StringVar(value="Siguiente Ajuste")
        algorithm_combo = ttk.Combobox(left_controls, textvariable=self.algorithm_var,
                                      values=["Siguiente Ajuste", "Peor Ajuste"], width=15)
    ...
    def demo_fragmentation(self):
        ...
        # Programar fase 2
        self.root.after(len(sizes) * 600 + 1000, self.demo_fragmentation_phase2)
    ...
    def execute_next_demo_step(self, index):
        ...
        if self.current_algorithm == "next_fit":
            self.memory_manager.next_fit(process_name, size)
        else:
    ...
    def toggle_pause(self):
        ...
        if self.demo_running:
            ...
            self.info_label.config(text=f"Demo {status} - " + self.info_label.cget("text"))
    ...
    def add_process(self):
        ...
        try:
            ...
            if self.current_algorithm == "next_fit":
                result = self.memory_manager.next_fit(process_name, size)
            else:
    ...
    def update_display(self):
        ...
        algorithm_name = "Siguiente Ajuste" if self.current_algorithm == "next_fit" else "Peor Ajuste"
        ...
        stats_text = f"{algorithm_name} | Usado: {used_memory}/50 ({usage_percentage:.1f}%) | "
